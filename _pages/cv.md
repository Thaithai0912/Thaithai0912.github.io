---
layout: single
title: "CV"
permalink: /cv/
author_profile: false
redirect_from:
  - /resume
---

{% include base_path %}

[Download resume PDF]({{ base_path }}/files/zhen-leng-thai-resume.pdf){: .btn}

Education
======

* M.S. in Computer Science and Technology, Tsinghua University, 2024-2026
  * THUNLP Lab, advised by Prof. Zhiyuan Liu
  * GPA: 3.9/4.0
  * Chinese Government Scholarship
* B.S. in Computer Science and Technology, Tsinghua University, 2020-2024
  * GPA: 3.85/4.0
  * Outstanding Graduate of Department of Computer Science and Technology

Research interests
======

* Large language model architecture
* Long-context language modeling
* Efficient attention mechanisms
* LLM pretraining, scaling, data engineering, and evaluation
* Practical AI systems and real-world impact

Selected experience
======

* THUNLP + ModelBest Inc., LLM Algorithm Researcher Intern, February 2024-June 2026
  * Worked on MiniCPM-SALA hybrid attention for efficient long-context inference.
  * Engineered tokenizer and data compression logic for the MiniCPM series.
  * Optimized hyperparameter frameworks and scaling-law analysis for small model performance.
* Tencent Technology Co., LLM Algorithm Engineer Intern, June 2023-September 2023
  * Worked on agent post-training, intent understanding, and backend tool utilization.

Selected projects and publications
======

* Hybrid Linear Attention Done Right
  * AdaptFM Workshop; EMNLP under review.
  * Authors: Yingfa Chen, **Zhen Leng Thai***, Zihan Zhou, Zhu Zhang, Xingyu Shen, Shuo Wang, Chaojun Xiao, Xu Han, Zhiyuan Liu.
  * Hybrid sparse/linear attention architecture for efficient long-context modeling in MiniCPM-SALA.
* Between a Rock and a Hard Place: The Tension Between Ethical Reasoning and Safety Alignment in LLMs
  * ACL 2026.
  * Authors: Shei Pern Chua, **Zhen Leng Thai**, Kai Jun Teh, Xiao Li, Qibing Ren, Xiaolin Hu.
  * Safety alignment and ethical reasoning research for complex moral reasoning settings.
* StateX: Enhancing RNN Recall via Post-training State Expansion
  * ACL 2026 Findings.
  * Authors: Xingyu Shen, Yingfa Chen, **Zhen Leng Thai**, Xu Han, Zhiyuan Liu, Maosong Sun.
  * Post-training state expansion for improving recurrent model recall.
* Cost-Optimal Grouped-Query Attention for Long-Context Modeling
  * EMNLP 2025.
  * Authors: Yingfa Chen, Yutong Wu, Chenyang Song, **Zhen Leng Thai**, Xingyu Shen, Xu Han, Zhiyuan Liu, Maosong Sun.
  * Cost-aware grouped-query attention design for long-context modeling.
* DecorateLM: Data Engineering Through Corpus Rating, Tagging, and Editing with Language Models
  * EMNLP 2024.
  * Authors: Ranchi Zhao, **Zhen Leng Thai***, Yifan Zhang, Shengding Hu, Yunqi Ba, Jie Zhou, Jie Cai, Zhiyuan Liu, Maosong Sun.
  * Data engineering pipeline for pretraining corpora via rating, tagging, and editing.
* ∞Bench: Extending Long Context Evaluation Beyond 100K Tokens
  * ACL 2024.
  * Authors: Xinrong Zhang, Yingfa Chen, Shengding Hu, Zihang Xu, Junhao Chen, Moo Khai Hao, Xu Han, **Zhen Leng Thai**, Shuo Wang, Zhiyuan Liu, Maosong Sun.
  * Long-context benchmark extending model evaluation beyond 100K tokens.
* OlympiadBench: A Challenging Benchmark for Promoting AGI with Olympiad-Level Bilingual Multimodal Scientific Problems
  * ACL 2024.
  * Authors: Chaoqun He, Renjie Luo, Yuzhuo Bai, Shengding Hu, **Zhen Leng Thai**, Junhao Shen, Jinyi Hu, Xu Han, Yujie Huang, Yuxiang Zhang, Jie Liu, Lei Qi, Zhiyuan Liu, Maosong Sun.
  * Bilingual multimodal scientific reasoning benchmark based on Olympiad-level problems.
* MiniCPM: Unveiling the Potential of Small Language Models with Scalable Training Strategies
  * COLM 2024.
  * Authors: Shengding Hu, Yuge Tu, Xu Han, Chaoqun He, Ganqu Cui, Xiang Long, Zhi Zheng, Yewei Fang, Yuxiang Huang, Weilin Zhao, Xinrong Zhang, **Zhen Leng Thai**, Kaihuo Zhang, Chongyi Wang, Yuan Yao, Chenyang Zhao, Jie Zhou, Jie Cai, Zhongwu Zhai, Ning Ding, Chao Jia, Guoyang Zeng, Dahai Li, Zhiyuan Liu, Maosong Sun.
  * Scalable training strategies for capable small language models.

\* denotes co-first author. I publish primarily as **Zhen Leng Thai**; my Chinese name is **Zhenning Dai** / 戴振宁.

Teaching and invited talks
======

* Teaching Assistant, Natural Language Processing, 2025
* Teaching Assistant, Natural Language Processing, 2026
* Guest Speaker, Towards Artificial Intelligence, 2026

Awards
======

* Chinese Government Scholarship, 2024-2026
* Outstanding Graduate of Department of Computer Science and Technology, Tsinghua University
* Beijing Government Scholarship, 2020-2024
* Tsinghua University Academic Excellence Award, 2021
* UTAR Mathematics Competition 2021, individual and group champion
* Champion of ACKUM Science and Maths Competition
* Second Runner Up of Xiamen University Malaysia Mathematics Competition

Skills and languages
======

* Technical: Natural Language Processing, model architecture, long-context modeling
* Languages: Chinese, English, Malay, Cantonese

Interests
======

Music, movies, and sports. I enjoy creative and curious conversations about how AI can become useful, trustworthy, and genuinely helpful.

Contact
======

* Email: [thaizhenleng123@gmail.com](mailto:thaizhenleng123@gmail.com)
* Google Scholar: [Thai Zhen Leng](https://scholar.google.com/citations?user=9uaxvAkAAAAJ&hl=en)
